...
RSpec.describe "Forecast object" do
  let!(:current) {
    CurrentWeather.new({dt: 1655095313,
                                      sunrise: 1655033490,
                                      sunset:  1655087299,
                                      temperature: 85.6,
                                      feels_like: 95.6,
  ...
  let!(:daily) {
    DailyWeather.new({dt: 1655095313,
                                  sunrise: 1646573135,
                                  sunset:  1646614624,
                                  max_temp: 85.6,
                                  min_temp: 45.6,
  ...
  let!(:hourly) {
    HourlyWeather.new({dt: 1655095313,
                                    temperature: 85.6,
                                    conditions: "overcast clouds",
                                    icon: "04d"})
  }
  ...
  it "returns a forecast object with the correct current weather object and data" do
    ...
    expect(current_weather.datetime).to eq("2022-06-12 ")
    expect(current_weather.sunrise).to eq("2022-06-12 ")
    expect(current_weather.sunset).to eq("2022-06-12")
  ...
  it "returns a forecast object with the correct daily weather array, objects and data" do
    ...
    expect(daily_weather.sunrise).to eq("2022-06-12 ")
    expect(daily_weather.sunset).to eq("2022-06-12")
  ...
  it "returns a forecast object with the correct hourly weather array, objects and data" do
    ...
    expect(hourly_weather.time).to eq("16:27:13")
